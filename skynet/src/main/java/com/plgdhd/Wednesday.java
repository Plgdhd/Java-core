...
public class Wednesday implements Runnable {
    ...
    private final List<Parts> wednesdayPartsStorage = new ArrayList<>();
    private int robotsCount = 0;
    private final AtomicBoolean running;
    private final Random rnd = new Random();

    public Wednesday(List<Parts> factoryStorage, AtomicBoolean running) {
    ...
    private void buildRobots() {
        ...
        while (haveEnoughDetails) {
            Map<Parts, Long> detailsCount = wednesdayPartsStorage.stream()
    ...
    public void removePart(Parts part, int code) {
        for (int i = 0; i < code; ++i) {
            wednesdayPartsStorage.remove(part);
        }
    }
    ...
    public void run() {
            ...
            synchronized (factoryStorage) {
                for (int i = 0; i < 5; ++i) {
                    if (!factoryStorage.isEmpty()) {
                        ...
                        wednesdayPartsStorage.add(factoryStorage.remove(partIndex));
                    }
                }
            }
            buildRobots();
        ...
        System.out.println("Wednesday built: " + robotsCount);
    }
